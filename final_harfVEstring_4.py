...
# arbitrary string search geliştirildi. Current stringe göre

# upgrade 5 için database görüntülemesi ve numaralandırılmış bir şekilde seçim
# ve current stringin değişitirlmnesi
# büyük bir olasılıkla yeni bir seçenekl eklenecek veya menü
...
class StringMuhendisi():
    ...
    #stringi parçala noktadan sonraki harfleri büyük harf yap
    #split ". " separator
...
class arbitraryPatternSearch():
    def __init__(self,string):
        self.pattern = input("lütfen aramak istediğiniz pattern stringi giriniz...")
        self.string  = string

    def arbitraryPatternSearchMenu(self):
        print("Arbitrary String Pattern Aramaya hoşgeldiniz...")
        print("""0=pattern var mı ?
        1=patternden kaç tane var ... 
        2=menüye dön""")
    def patternFound(self):
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!
        if search(self.pattern,self.string):
            print("Pattern Found !!!")
        else:
            print("pattern cannot be found ...")
    def countPattern(self):
        print("Matched pattern number : ",len(findall(self.pattern,self.string)))
...
if __name__ == '__main__':
    ...
    # 14= vertiabanindaki stringleri göster
    ...
    while (True):
        ...
        if selection == 1:
            ...
            arbitraryQuery=arbitraryPatternSearch(stringg)
            while True:
                arbitraryQuery.arbitraryPatternSearchMenu()
                query_selection = input("lütfen seçim yapınız...")
                if query_selection == "0":
                    arbitraryQuery.patternFound()
                    continue
                elif query_selection =="1":
                    arbitraryQuery.countPattern()
                    continue
                elif query_selection == "2":
                    break
            continue
        else :
            print("Geçersiz seçim...Lütfen 0-14 arasında bir seçim yapınız.")
